...
- `(public)/join-us/page.tsx` — recruitment status. Deliberately static, not CMS-backed: it's one paragraph with a date, and a `home_sections` row would be one more thing an editor has to find before the page makes sense. It earns a content model when recruitment actually opens and it becomes a real flow. The background is blurred **on the element with `scale-110`**, not via a `backdrop-filter` overlay — `blur()` leaves a soft transparent edge at the element's own bounds, and the scale hides it.
